...
class SourceNode(Pubsub):
    def __init__(self, source: Source, subs: list[Subscriber] = None, uuid: UUID = None):
    ...
    def on_update(self, old_data: PydanticModel, new_data: PydanticModel):
        if isinstance(old_data, Wire) and isinstance(new_data, Wire):
            ...
            for i, wire in enumerate(self._model.wires):
                if wire.uuid == new_data.uuid:
                    self._model.wires[i] = new_data
                    return
            raise LookupError
        raise TypeError
